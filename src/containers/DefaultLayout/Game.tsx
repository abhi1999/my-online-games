...
interface IGameState {
  validSession:boolean, showAlert:boolean, shouldNavigateToGameCard:boolean
}
class Game extends Component<IGameProps, IGameState> {
  ...
  public componentWillReceiveProps(newProps:IGameProps){
    ...
    if(gamesReducer.validGame &&  this.state.shouldNavigateToGameCard){
        this.props.history.push('/Bingo/Games/'+gamesReducer.activeGameId +"/Participant/"+ gamesReducer.activeParticipantId)
    }
  ...
  private clickOnParticipant = (gameId:string, participantId:string)=>{
    this.setState({shouldNavigateToGameCard:true})
    this.props.onParticipantClicked(gameId, participantId, participantId);
  }
  
  public render() {
    
    if(!this.state.validSession && this.state.showAlert){
    ...
    return (
        <div>
                <h5 className="display-4">{game?.name}</h5>
